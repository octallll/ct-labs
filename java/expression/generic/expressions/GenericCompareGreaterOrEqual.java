...
public class GenericCompareGreaterOrEqual<T> extends GenericBinaryOperation<T> {
    public GenericCompareGreaterOrEqual(Expressions<T> leftOperation, Expressions<T> rightOperation, Operator<T> operator) {
        super(leftOperation, rightOperation, operator);
    }

    @Override
    public T calculateResultOfOperation(T leftResult, T rightResult) {
        return operator.compareGreaterOrEqual(leftResult, rightResult);
    }
